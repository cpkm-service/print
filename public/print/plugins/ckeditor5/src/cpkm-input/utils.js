...
export function prepareIntegrations(editor) {
    ...
    const cpkmInputUIPlugin = editor.plugins.get('CpkmInputUI');
    const PREDEFINED_INTEGRATIONS = {
        'insertInputWidth': createLabeledInputView(editor.locale)
    };
    if (!panelItems) {
    ...
    if (panelItems.find(item => item === 'openCKFinder') && editor.ui.componentFactory.has('ckfinder')) {
        ...
        ckFinderButton.delegate('execute').to(cpkmInputUIPlugin, 'cancel');
...
export function createLabeledInputView(locale) {
    ...
    labeledInputView.fieldView.placeholder = '100';
